...
async function initializeWatchPage() {
	try {
		const response = await secureFetch(`http://localhost:5000/api/videos`);
...
async function handleSubscribeToggle() {
	...
	try {
		const response = await secureFetch(
			`http://localhost:5000/api/users/subscribe/${videoCreatorId}`,
			{
				method: "POST",
			},
		);
...
async function handleVideoReport() {
	...
	try {
		const response = await secureFetch(
			`http://localhost:5000/api/videos/report`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
...
async function logToWatchHistory() {
	try {
		await secureFetch(`http://localhost:5000/api/videos/history`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
